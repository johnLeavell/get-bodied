class WorkoutExercisesController < ApplicationController
  ...
  def update
    ...
    if the_workout_exercise.valid?
      the_workout_exercise.save
      redirect_to("/workout_exercises/#{the_workout_exercise.id}",
                  { notice: 'Workout exercise updated successfully.' })
    else
      redirect_to("/workout_exercises/#{the_workout_exercise.id}",
                  { alert: the_workout_exercise.errors.full_messages.to_sentence })
    end
  end
